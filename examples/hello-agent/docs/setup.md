...
```bash
az ad sp create-for-rbac --name hello_agent-staging-sp --skip-assignment
az ad sp create-for-rbac --name hello_agent-prod-sp --skip-assignment
```
...
3. **Apply the Unity Catalog grants from step 2b**, substituting the SP's Application ID for `<identity>`. Use backticks around the Application ID in SQL:
   ...
   ```sql
   GRANT USE CATALOG ON CATALOG hello_agent_staging TO `<staging-app-id>`;
   GRANT CREATE SCHEMA ON CATALOG hello_agent_staging TO `<staging-app-id>`;
   ```
