...
DEFAULT_PARAM_VALUES = {
    ...
    # input_schema_name is explicitly set in test context (UUID), so its default is not tested here
...
DEFAULT_PARAMS_AZURE = {
    "input_cloud": "azure",
    "input_databricks_dev_workspace_host": "https://adb-xxxx.xx.azuredatabricks.net",
...
DEFAULT_PARAMS_AWS = {
    "input_cloud": "aws",
    "input_databricks_dev_workspace_host": "https://your-dev-workspace.cloud.databricks.com",
...
DEFAULT_PARAMS_GCP = {
    "input_cloud": "gcp",
    "input_databricks_dev_workspace_host": "https://your-dev-workspace.gcp.databricks.com",
...
def test_no_databricks_doc_strings_before_project_generation():
    template_dir = pathlib.Path(__file__).parent.parent / "template"
    test_paths = [os.path.join(template_dir, path) for path in paths(template_dir)]
    # Prohibit old cloud-specific doc URLs (Databricks docs are now unified at docs.databricks.com)
    assert_no_disallowed_strings_in_files(
        file_paths=test_paths,
        disallowed_strings=[
...
def test_generate_project_agent_structure(
    ...
    generate(tmpdir, databricks_cli, context=context)
    # Flat structure: bundle root = project root (no project subdirectory)
    project_dir = tmpdir / TEST_PROJECT_NAME
    if setup_cicd_and_project != "CICD_Only":
        # Flat structure: notebooks are directly inside their parent step directory
        assert (project_dir / "agent_development" / "Agent.py").exists()
        assert (project_dir / "agent_development" / "app.py").exists()
        assert (project_dir / "agent_development" / "AgentEvaluation.py").exists()
        assert (project_dir / "data_preparation" / "DataIngestion.py").exists()
        assert (project_dir / "data_preparation" / "DataPreprocessing.py").exists()
        assert (project_dir / "data_preparation" / "VectorSearch.py").exists()
        assert (project_dir / "agent_deployment" / "ModelServing.py").exists()
        # Verify old nested subdirectories are gone
        assert not (project_dir / "agent_development" / "agent").exists()
        assert not (project_dir / "agent_development" / "agent_evaluation").exists()
        assert not (project_dir / "data_preparation" / "data_ingestion").exists()
        assert not (project_dir / "data_preparation" / "data_preprocessing").exists()
        assert not (project_dir / "data_preparation" / "vector_search").exists()
        assert not (project_dir / "agent_deployment" / "model_serving").exists()
