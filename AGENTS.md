...
6. **Integrate with coding assistants, don't replace them.** The project
   ...
   governance, and lifecycle surface above them. Every workflow must be
   completable without any specific assistant — through the CLI or by editing
   DAB files directly. Plugins and assistants are accelerators, not
   prerequisites.
...
The project has two channels, both producing or operating on the same scaffold:

- **DAB template (canonical).** A pure-CLI scaffold generated by `databricks
  bundle init`. Runs in any environment with the Databricks CLI. Produces the
  bundle layout, CI/CD wiring, and scaffold contract.
- **agentops-stacks plugin.** Portable coding-assistant skills (SKILL.md +
  installer scripts) for Claude Code, Cursor, and Genie Code. Provides
  authoring help on top of an existing scaffold — applying eval, governance,
  and monitoring patterns, reviewing changes against project priorities, and
  proposing the next step in the lifecycle. The plugin is optional; the
  template stands on its own.

Both channels write to a shared scaffold contract — `.agentops-stacks/manifest.yml`
